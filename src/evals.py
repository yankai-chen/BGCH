...
class bgch_loss:
    ...
    def stage(self, user_index, pos_index, neg_index):
        loss1, loss2, reg_loss = self.model.loss(user_index, pos_index, neg_index)
        loss = loss1 + loss2
        ...
        return loss.cpu().item(), loss1.cpu().item(), loss2.cpu().item()
...
def Train(dataset, model, epoch, loss_f, neg_ratio=1, summarizer=None):
    ...
    for batch_i, (b_user_idx,
        ...
        loss_all_i, loss1_i, loss2_i = loss_f.stage(b_user_idx, b_pos_item_idx, b_neg_item_idx)
        avg_loss += loss_all_i
        loss1 += loss1_i
        loss2 += loss2_i
        if board.args.tensorboard:
            ...
            summarizer.add_scalar(f'bfs/loss2', loss2, epoch * num_batch)
    ...
    utils.timer.zero()
    #
    info = f'all_loss:{avg_loss: .3f} | loss1:{loss1: .3f} | loss2:{loss2: .3f} | time cost-{time_info}|'
